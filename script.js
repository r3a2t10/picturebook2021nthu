...
function myFunction() {
	...
	//createValueButtons(S,E);

	// Initial 2D Array
	var arr = Array(E-S).fill('00000');
	...
	$('.value-btn').mousedown(function () {
		...
		if($(this).hasClass('active')) {
			...
			var active_num = findValueWeight(prompt_values, $(this).text());
			arr[res-S] = setCharAt(arr[res-S],active_num-1,'0');
		} 
		else {
			$(this).addClass('active');
			var active_num = findValueWeight(prompt_values, $(this).text());
			arr[res-S] = setCharAt(arr[res-S],active_num-1,'1');
		}
		document.getElementById("text-val").value = arr;
		//console.log(arr);
	})
...
function createPromptItems() {
	...
	for (var i = 0; i < prompts.length; i++) {
		...
		var prompt_img = document.createElement('img'); 
		prompt_img.src = createImgPromptItems();
		prompt_img.width = "300";
...
function createValueButtons(start,end) {
	for (var li_index = start; li_index < end; li_index++) {
		var group = document.createElement('div');
		group.className = 'btn-group btn-group-justified';
...
$('#submit-btn').click(function () {
	...
	document.getElementById('results').innerHTML = '<b>You are great!</b><br><br>';
